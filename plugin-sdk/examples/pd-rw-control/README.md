...
$$
q_\text{err} = q_\text{target}^{-1} \otimes q_\text{current}
$$

$$
\boldsymbol{\theta}_\text{error} \approx 2 \cdot \text{vec}(q_\text{err}) \quad (\text{半球選択済み})
$$

$$
\boldsymbol{\tau} = -K_p \boldsymbol{\theta}_\text{error} - K_d \boldsymbol{\omega}
$$
