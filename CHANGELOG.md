...
- A TOML config may give `epoch` as a datetime, without quotes:
  ...
  expected a string`, while the quoted form and the same line in YAML ran. The datetime is now read as its text and checked as a quoted epoch is,
  so an offset such as `+09:00` is refused with the same `invalid epoch`
  error. An `epoch` of another type names `epoch` in its error in JSON too.
...
- `run` writes each output time once. With a decimal output interval the last
  ...
  the controlled run already counted them, and a time that lands a rounding
  below the end is the end itself: within 1e-9, or a few ulps of the end at
  large magnitudes. The controlled run wrote its end twice the same way when
  its last boundary landed just below it — with `--output-interval 0.3
  --duration 0.9` it sampled at `3 × 0.3 = 0.8999999999999999` and its tail
  recorded 0.9, both `0.900` — and now samples 0.9 once.
...
- `run` and `serve` stop with `Error:` and exit 1 when an orbit or a space
  ...
  every connection until it was restarted: a `start_simulation` whose NORAD
...
- The OMM parsers read `MEAN_ELEMENT_THEORY = SGP/SGP4` as SGP4. CelesTrak's
  ...
  `orts run --omm` on it exited 1. CCSDS 502.0-B-3 uses `SGP/SGP4` to mark an
  OMM made from a TLE (its theory table lists `SGP` and `SGP4` separately), and
  distributed TLE data are all generated by SGP4, so the value is read as SGP4
  like a TLE; the sources are linked from the parser's code. It now parses to
  the same record as the XML and JSON, and `orts run` gives the same CSV rows as
  from the ISS 3LE. `SGP` alone,
