...
async function measureFps(
	...
	await page.evaluate(() => {
		function m(now: number) {
			if ((window as any).__ftLast > 0)
				(window as any).__ft.push(now - (window as any).__ftLast);
...
test("isolate: charts vs 3D", async ({ page }) => {
	...
	console.log(
		`Charts+3D:       ${withCharts.fps} fps (${withCharts.avgMs}ms avg)`,
	);
	console.log(
		`3D only:         ${withoutCharts.fps} fps (${withoutCharts.avgMs}ms avg)`,
	);
	console.log(
		`Charts only:     ${without3D.fps} fps (${without3D.avgMs}ms avg)`,
	);
	console.log(
		`Neither:         ${withoutBoth.fps} fps (${withoutBoth.avgMs}ms avg)`,
	);
	console.log("");
});
