...
describe("createOrbitSchema", () => {
  ...
  it("toRow does not produce undefined for any field", () => {
    ...
    const point = {
      t: 10, x: 6778, y: 100, z: 0, vx: -0.1, vy: 7.669, vz: 0,
      a: 6780, e: 0.001, inc: 0.9, raan: 3.14, omega: 1.0, nu: 0.5,
    };
    const row = schema.toRow(point);
    for (let i = 0; i < row.length; i++) {
      expect(row[i], `column ${schema.columns[i].name} should not be undefined`).not.toBeUndefined();
    }
  });
