...
- **境界の値は導関数と同じ segment で読む。** segment の間だけ値を保持する部分 (指令された燃焼、
  ...
  誤った側に収束するか、局在化に失敗する
- **境界の値は残余である。** 境界の手前で正、境界上でちょうど 0、越えると負にする。到達は残余が
  尽きる向きの 1 方向で、上がって境界に入る量は符号を反転して書く。越えた側が 1 つに決まるので、
  「刻み始める state が既に境界を越えている」場合 (探索では見つけられない) を判定できる。
...
- **境界は effector が申告する。** ホイール 1 つにつき到達 2 つ (上限・下限) と解除 1 つで、
  値はそれぞれ残余 $h_{max} - h$、$h + h_{max}$、$\nu$ である。aux と mode のどこに自分の
  ブロックがあるかを知っているのは effector を持つ系なので、系が申告を平坦化して
  `DeclaredBoundary` にまとめる
