...
- ただし、値が尽きて戻るまでが 1 ステップに収まる event も、**その 2 つの零点のあいだで別の event の
  交差が見つかる場合には見つけられる**。根を 1 つ求めた時点の値をもう一度読み、そこで始点との符号差が
  生じている event を候補に加え、短くなったステップで $lo = 0$ から探索し直す。報告するのは最終的な
  ...
  なるため、件数は 2 のまま変わらない)。求めた根が値の尽きる前や戻ったあとにあるときは候補が増えないので、
