...
- **太陽を遮る天体は一覧で持つ**: 遮蔽体は中心天体 1 つではなく `Vec<OccultingBody>`。月周回の衛星は地球の影にも入る (2026 年で年 4.0〜7.7 時間、月食の日)。遮蔽の幾何は天体ごとに持つ。円柱近似は周回している天体では食の長さの 0.5% だが、月周回から見た地球では 1.86 倍になる。合成は視円板の位置関係で 3 通りに分ける。離れていれば割合を足す(厳密)。一方が他方の内側にあれば大きい方を採る(厳密、手前の天体が奥の天体ごと隠している)。どちらでもない重なりでは `a + b - ab` を採る。厳密な答えは 2 円の和集合を第 3 の円で切った面積で、これは計算していない。この値は大きい方と和の間に入り、部分食 2 つから皆既を作らない(0.6 と 0.6 を足すと皆既になるが、実際にはどちらも部分食で和も皆既ではない)。3 番目の場合は既定の一覧でも起きる。月食の間は太陽が地球の後ろにあるので、衛星が月の terminator を通るとき地球の小さい視円板が月の輪郭をまたぐ。3 天体以上では、各天体を「実際に視円板が触れている天体」だけに対して割り引く。連結成分でまとめると、A と B が触れ B と C が触れて A と C が離れている場合に、C を A のぶんまで割り引いてしまう。中心天体の既定の幾何は consumer が決める。SRP 2 つは円柱、sun sensor は円錐で、これを揃えると固定済みの数値が動くため
