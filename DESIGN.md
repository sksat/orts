...
推進剤は宇宙機に 1 つのプールで、床 (dry mass) も 1 つである。独立タンク・二液式は適用範囲外と
書く。現状は `ThrusterSpec` と `ThrusterAssemblyCore` の 2 箇所が床を持ち、どちらも評価時に
`state.mass <= dry_mass` で出力をゼロにしている。この比較は 2 つの理由で置き換える。
