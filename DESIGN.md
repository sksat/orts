...
十分広いステップは質量 0 以下のステージに達する。力を加速度に変える model は全てそこで質量で割るので
(thruster、パネルの drag / SRP)、`F/m` は無限大になり、walk は捨てる予定の state で失敗する。
**その判定は系が 1 度行う**: 質量が正でない state では model を 1 つも評価しない (model ごとに
自衛させると、thruster が自衛してもパネルが無限大を作る)。物理的な意味はない state であり、
二分探索が必要としているのは有限な値だけである。
