...
- **どちらのモデルも入射元を向く面を選ぶ**: SRP と大気抵抗は、力を受ける面として入射元を向いた面を選ぶ。SRP は `cos θ = n̂·ŝ` (`ŝ` は機体から太陽への方向)、大気抵抗は `cos θ = n̂·v̂_rel` (`v_rel` は大気に対する機体の速度で、機体の系では気体が `+v_rel` の側から来る)。遮蔽の判定も同じ向きを使い、入射元の側に立つパネルだけが影を落とす。この規約がコメントにしか書かれていない間、2 つのモデルで面の選び方が逆になっていた (#416)
- **パネルの輪郭は optional**: パネルは面積と法線と圧力中心で表し、面内の輪郭は持たなくてよい。力の法則が輪郭を使わないからである。輪郭を書いたパネル同士だけが遮蔽の判定対象になる。遮蔽は 1 枚のパネルに完全に隠れる場合だけを扱う。2 枚の和で覆われる配置は照射扱いのままで、一部だけ隠れる場合は圧力中心が照射領域の重心に移って定数の `cp_offset` では表せない
