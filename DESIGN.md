...
- **境界の値は残余である。** 境界の手前で正、境界上でちょうど 0、越えると負にする。到達は残余が
  ...
  「歩き始めの state が既に境界を越えている」場合 (探索では見つけられない) を判定できる。
...
- **境界で変えた state は観測と終了判定に回す。** 境界処理が state を動かしたら、呼び出し側が
  ...
  終了判定に渡してから再開する (遷移の途中の state は観測しない)。歩き始めの settle でも同じで、
  「呼び出し側は判定済み」と言われた state でも、動かしたなら判定をやり直す
- **歩くループは 1 つ。** `orts::boundary::walk_to_target` が「歩く → 境界で止まる → state を
  境界に載せる → モードを進める → 再開」を持ち、独立群・結合群・CLI の制御付き経路がそれを呼ぶ。
...
- **契約が成立する範囲は、この walk を通る経路である。** `Integrator::integrate` で系を直接
...
(thruster、パネルの drag / SRP、並進力を返す effector)、`F/m` は無限大になり、walk は捨てる予定の
state で失敗する。
...
歩き始めの settle では拾えない)、床より上は燃料あり。`dry_mass > 0` を必須にする理由は上の
