...
| Phase D | 姿勢制御検証基盤: DecoupledAttitudeSystem, PD制御, AttitudeReference |

#### Phase D 詳細: 姿勢制御検証基盤

**DecoupledAttitudeSystem**: 姿勢のみ積分し、軌道と質量は外部関数から注入する system。
`DecoupledContext` 型が全 capability (`HasAttitude + HasOrbit + HasMass`) を持ち、
あらゆる `Model<S>` がコンパイルレベルで動作する。ただし `torque_body` のみ使用し、
`acceleration_inertial` / `mass_rate` は無視する（debug build で非ゼロを警告）。

**AttitudeReference trait**: 目標姿勢（body-to-inertial quaternion + 目標角速度）を生成。
`InertialPointing`（固定慣性姿勢）、`NadirPointing`（地球指向）を提供。

**PD姿勢制御則**: right-invariant quaternion error (`q_error = q_current ⊗ q_target⁻¹`) を使用し、
姿勢誤差を body frame で直接表現。`omega_target` は `q_error` の回転行列で body frame に変換。
- `InertialPdController`: `impl<S: HasAttitude> Model<S>` — 全 system で動作
- `TrackingPdController<R>`: `impl<S: HasAttitude + HasOrbit, R: AttitudeReference> Model<S>` — DecoupledAttitudeSystem / SpacecraftDynamics で動作
