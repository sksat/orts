...
| Phase B' | Capability-based `Model<S>` trait 移行: ForceModel/TorqueModel/LoadModel → 統一 `Model<S>` |
| Phase C | 複数衛星: group + scheduler モジュール、CLI 簡素化 |
| Phase D | 姿勢制御検証基盤: DecoupledAttitudeSystem, Controller/Actuator 分離, RW/磁気トルカ/PD制御 |
