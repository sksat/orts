...
```
orts run                                     # シミュレーション実行（CWD の orts.toml を自動検出）
orts run --format csv                        # CSV を stdout に出力
orts serve [OPTIONS]                         # WebSocket サーバー
orts convert <input> --format csv            # フォーマット変換（rrd → csv）
```

### run オプション
- `--sat <spec>` — 衛星仕様（例: `altitude=400,inclination=51.6`）
...
| Phase E | 姿勢 viewer 接続: serve→WS→viewer パイプラインに姿勢データ追加、SimGroup enum。viewer は衛星メッシュ姿勢 + body axes を描画。GLTF モデルがない衛星は向きの分かる XYZ cube marker（面色=body 軸、球ではなく）で表示し、形状は viewer 側で選択可（全体デフォルト + 衛星ごと override、sphere / xyz-cube）。レンダリング姿勢を `__debug_get_sat_world_quat` 経由で E2E 回帰テスト済み **実装済み** |
