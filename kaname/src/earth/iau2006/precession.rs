...
/// `kaname/tests/iau2006_vs_erfa.rs`).
...
/// `iauObl06` and ERFA's `eraObl06` use the correct value, and kaname
/// matches them so that the ERFA oracle fixture in
/// `kaname/tests/iau2006_vs_erfa.rs` passes to 10⁻¹² rad at `|t| ≤ 1`
...
mod tests {
    ...
    // `kaname/tests/iau2006_vs_erfa.rs` which loads the fixture at
