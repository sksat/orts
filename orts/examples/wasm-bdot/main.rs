...
//! cd plugin-sdk/examples && cargo +1.91.0 component build -p orts-example-plugin-bdot-finite-diff --release && cd -
//!
//! # Run the simulation sweep:
//! cargo run --example wasm-bdot --features plugin-wasm --release
//! ```
//!
//! Outputs CSV files in `plugin-sdk/examples/bdot-finite-diff/`:
//!
//! - `sim_<model>_gain_<gain>_omega_<omega>.csv`
//!
...
use tobari::magnetic::{Igrf, MagneticFieldModel, TiltedDipole};
...
const T_END: f64 = 55500.0; // ~10 orbits at 500 km altitude

fn main() {
    let gains = [1e5, 1e6, 1e7];
    ...
    let output_dir: PathBuf = format!(
        "{}/../plugin-sdk/examples/bdot-finite-diff",
        env!("CARGO_MANIFEST_DIR")
    )
    .into();
    ...
    for &gain in &gains {
        for &omega in &initial_omegas {
            ...
            print_and_run(&pre, &output_dir, &label, gain, omega, || Igrf::earth());
        }
    }

    println!("\nDone. CSV files written to {}", output_dir.display());
    println!("Plot with: cd plugin-sdk/examples/bdot-finite-diff && python plot.py");
}

fn print_and_run<F: MagneticFieldModel + 'static>(
    pre: &orts::plugin::wasm::bindings::PluginPre<orts::plugin::wasm::host_state::HostState>,
    output_dir: &std::path::Path,
    label: &str,
    gain: f64,
    initial_omega_mag: f64,
    make_field: impl Fn() -> F,
) {
    println!("Running: {label} ...");
    let csv_path = output_dir.join(format!("sim_{label}.csv"));
    let trajectory = run_case(pre, label, gain, initial_omega_mag, &make_field);
    write_csv(&csv_path, &trajectory);
    let final_omega = trajectory.last().unwrap().1;
    println!(
        "  {label} -> |omega| {initial_omega_mag:.6} -> {final_omega:.6} rad/s ({:.1}% reduction)",
        (1.0 - final_omega / initial_omega_mag) * 100.0,
    );
}
...
fn run_case<F: MagneticFieldModel + 'static>(
    pre: &orts::plugin::wasm::bindings::PluginPre<orts::plugin::wasm::host_state::HostState>,
    label: &str,
    gain: f64,
    initial_omega_mag: f64,
    ...
    let omega0 = initial_omega_mag / 3.0_f64.sqrt();
    ...
    let mut sensor_bundle = SensorBundle {
        magnetometers: vec![Magnetometer::new(Arc::clone(&field_arc))],
    ...
    while t < T_END - 1e-12 {
        ...
        let actuator = CommandedMagnetorquer::new(mtq_moment_vec3(&bundle), make_field());
