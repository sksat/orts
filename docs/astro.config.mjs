  ...
  integrations: [
    ...
    starlight({
      ...
      plugins: [
        ...
        // Emit /llms.txt, /llms-full.txt, /llms-small.txt so coding agents and
        // LLM tooling can ingest the docs (https://llmstxt.org/). The plugin is
        // by the Starlight lead maintainer (delucis); MIT.
        starlightLlmsTxt({
          projectName: "orts",
          description:
            "orts is a numerical computation and optimization platform primarily for orbital mechanics: a Rust simulation engine and `orts` CLI, plus a React + Three.js web viewer.",
          // The autogenerated rustdoc/typedoc API reference is large and mostly
          // boilerplate. Keep it out of the condensed llms-small.txt so that
          // version stays a high-signal overview (guides, per-crate overviews,
          // examples); it is still included in the complete llms-full.txt.
          exclude: ["**/api/**"],
        }),
      ],
      sidebar: [
