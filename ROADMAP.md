...
- **STL などのメッシュから形状を読み込む**: 今は `[[satellites.panels]]` に面を 1 枚ずつ書く。CAD が出す三角形メッシュを読んで、そこから SRP と大気抵抗の面を作れるようにする。`PanelOutline` を enum にしてあるので、三角形の variant を足す形で入る。面数が 1000 を超える規模になると、遮蔽判定の O(N²) と、面ごとの光学特性をどう与えるかが問題になる
