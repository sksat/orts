...
**スコープ外**: ケプラー要素（離心率, 軌道傾斜角等）は軌道力学の概念であり orts（orbital モジュール）に所属する。
arika は「地球と天球の幾何学」を扱い、「軌道の力学」は扱わない。
...
| Time scale | 種別 | 紐づく frame | arika での扱い |
|---|---|---|---|
...
UTC calendar arithmetic (「翌日同時刻」「翌月同日」) は arika の scope 外。必要になれば `CivilDuration`
相当の API を別途追加する。

### leap second は geodetic 現象

Earth rotation の secular slowdown (潮汐摩擦) が原因。leap second table を arika 内 compiled-in で持つ。
...
arika 自体は data file を持たない (WASM compatibility)。
