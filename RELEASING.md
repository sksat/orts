...
1. **GitHub Release (バイナリ配布)** — `.github/workflows/release.yml` が
   tag push で自動発火。`v*` tag を打つと draft release が作られ、
   x86_64-unknown-linux-gnu 向けの tarball + checksum が添付される。
...
# Actions が release.yml を発火、draft release を作成
gh run watch
...
- **Version mismatch**: tag 名と `Cargo.toml` version が不一致 →
  release.yml 冒頭の verify step で早期 fail。version bump 忘れ。
- **cargo-about が無い**: `cli/build.rs` が panic。CI runner 側で
  `taiki-e/install-action` で install されるはず。
- **viewer build 失敗**: pnpm step で fail、tarball は作られない。
