...
#### Fixed
- `RootSet::scan_step` が、根を 1 つ求めた時刻の値をもう一度読み、step の両端では符号差が
  なかった event がそこで bracket されていれば、step の始点から探索し直すようになった。
  値が尽きて戻るまでが 1 step に収まる event は両端が同符号なので、従来は見送られていた。
  その 2 つの零点のあいだに別の event の交差があれば、短くなった step では符号差が現れ、
  step 内で最も早い交差が報告される。候補は 1 回の探索のあいだ入れ替えずに増やす:
