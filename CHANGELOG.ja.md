...
- `PanelDrag` が、風下の面ではなく流れに向いた面に抵抗を出すようになった。`v_rel` は
  ...
  影も同じく風下側に落ちていた。今は気体が来る側に落ちる。変わるのは影の側だけで、
  遮蔽がどのパネルを落とすか、一部だけ隠れたパネルをどう扱うかは #424 のままである
  ([#407](https://github.com/sksat/orts/issues/407))。法線が対になる形状 (`cube` など閉じた箱) では力の
